...
import { BoringOwnable } from "./libraries/BoringOwnable.sol";
...
contract RULERRewarder is IRewarder, BoringOwnable {
	...
	modifier onlyMCV2 {
		require(msg.sender == address(MC_V2), "RULERRewarder::onlyMCV2: only MasterChef V2 can call this function.");
		_;
	}
	...
	constructor(
		...
		require(Address.isContract(address(_rewardToken)), "RULERRewarder: reward token must be a valid contract");
		require(Address.isContract(address(_MCV2)), "RULERRewarder: MasterChef V2 must be a valid contract");
	...
	function addPool(uint256 _pid, uint256 allocPoint) public onlyOwner {
		require(poolInfo[_pid].lastRewardBlock == 0, "RULERRewarder::add: cannot add existing pool");
